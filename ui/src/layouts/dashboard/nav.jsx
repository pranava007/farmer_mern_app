...
export default function Nav({ openNav, onCloseNav }) {
  ...
  const renderAccount = (
    ...
    >
      <Avatar src={previewURL || account.photoURL} alt="photoURL" />

      <Box sx={{ ml: 2 }}>
        <Typography variant="subtitle2">{userDetail?.name}</Typography>

        <Typography variant="body2" sx={{ color: 'text.secondary' }}>
          Role: {userRole === "1" ? "Farmer" : "User"}
        </Typography>
      </Box>
    </Box>
  );
  ...
  const renderMenu = (
    <Stack component="nav" spacing={0.5} sx={{ px: 2 }}>
      {navConfig.filter(item => item.role.includes(userRole)).map((item) => (
